...
def estimate_similar_transform(x, y):
    ...
    assert (
        y.shape[1] == n
    ), "numbers of points in x and y should be the same, but now they are:" + ",".join(
        [n, y.shape[1]]
    )
    ...
    assert (
        rank_cov_xy >= m - 1
    ), "rank (cov_xy) < m - 1, refer to Equations (40-43) in http://web.stanford.edu/class/cs273/refs/umeyama.pdf"

...
def generate_image(out_dir, gen_particles, base_image, base_particles):
    ...
    transform = sw.ImageUtils.createWarpTransform(
        base_particles, gen_particles, 2)
...
by Wenzheng Tao
    ...
    '''
    m, n = x.shape
    assert (y.shape[1] == n), 'numbers of points in x and y should be the same, but now they are:' + \
        ','.join([n, y.shape[1]])

...
class PCA_Embbeder(Embedder):
    ...
    def run_PCA(self, num_dim, percent_variability):
        ...
        data_matrix_2d = self.data_matrix.reshape(
            self.data_matrix.shape[0], -1).T  # flatten data instances and transpose
        mean = np.mean(data_matrix_2d, axis=1)
        centered_data_matrix_2d = (data_matrix_2d.T - mean).T
        trick_cov_matrix = np.dot(
            centered_data_matrix_2d.T, centered_data_matrix_2d) * 1.0/np.sqrt(N-1)
        ...
        for i in range(N):
            eigen_vectors[:, i] = eigen_vectors[:, i] / \
                np.linalg.norm(eigen_vectors[:, i])

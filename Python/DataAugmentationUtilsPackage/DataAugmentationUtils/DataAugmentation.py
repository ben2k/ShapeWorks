...
def point_based_aug(
    ...
    if orig_world_point_list is not None:
        ...
        world_get_local = [
            Utils.estimate_homogeneous_similar_transform(
                x=p.reshape((-1, 3)).T,
    ...
    PointEmbedder.write_PCA(
        out_dir + "PCA_Particle_Info/", "particles"
    ...
    for index in range(1, num_samples + 1):
        ...
        if orig_world_point_list is not None:
            ...
            gen_points = (
                world_get_local[i]
                @ Utils.get_homogeneous_coordinates(p.reshape((-1, 3)).T)
            )[:3, :].T
...
def generate_image(param):
    gen_image_path = Utils.generate_image(
        param["out_dir"],
        param["gen_points_path"],
        param["base_image_path"],
        param["base_particles_path"],
    )

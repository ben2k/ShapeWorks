...
def point_based_aug(out_dir, orig_img_list, orig_point_list, num_samples, num_dim=0, percent_variability=0.95, sampler_type="KDE", mixture_num=0, processes=1, orig_world_point_list=None):
    ...
    if orig_world_point_list is not None:
        ...
        PointEmbedder = Embedder.PCA_Embbeder(
            world_point_matrix, num_dim, percent_variability)
    else:
        PointEmbedder = Embedder.PCA_Embbeder(
            point_matrix, num_dim, percent_variability)
    num_dim = PointEmbedder.num_dim
    # write PCA info for DeepSSM testing
    PointEmbedder.write_PCA(out_dir + "PCA_Particle_Info/", "particles")
    ...
    for index in range(1, num_samples+1):
        ...
        if orig_world_point_list is not None:
            ...
            i = int(np.nanargmin(np.linalg.norm(
                p-world_point_matrix, axis=(1, 2))))
            # TODO add Randomness in the transformation from world to local particles
            gen_points = (
                world_get_local[i] @ Utils.get_homogeneous_coordinates(p.reshape((-1, 3)).T))[:3, :].T
            world_get_local_info['generated_particle__world_get_local_index'].append(
                i)
        else:
        ...
        if processes == 1:
            gen_image_path = Utils.generate_image(
                out_dir, gen_points_path, base_image_path, base_particles_path)
    ...
    Utils.make_CSV(out_dir + "TotalData.csv", orig_img_list, orig_point_list,
                   embedded_matrix, gen_image_paths, gen_points_paths, gen_embeddings)
    return num_dim


def generate_image(param):
    gen_image_path = Utils.generate_image(
        param['out_dir'], param['gen_points_path'], param['base_image_path'], param['base_particles_path'])

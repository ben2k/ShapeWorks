...
class Mixture_Sampler(Sampler):
    def fit(self, embedded_matrix, mixture_num):
        ...
        self.GMM = GaussianMixture(
            mixture_num, covariance_type='full', random_state=0)
    ...
    def selectClusterNum(self):
        ...
        models = [GaussianMixture(n, covariance_type='full', random_state=0).fit(
            self.embedded_matrix) for n in n_components]
        bic_min_index = np.argmin(
            np.array([m.bic(self.embedded_matrix) for m in models]))
        aic_min_index = np.argmin(
            np.array([m.aic(self.embedded_matrix) for m in models]))
...
class KDE_Sampler(Sampler):
    def fit(self, embedded_matrix):
        ...
        self.sigma_squared = np.mean(
            np.array(nearest_neighbor_dists))/embedded_matrix.shape[1]

...
def _loginAndGetAccessToken():
    ...
    if accessToken is None:
        ...
        if loginState is None:
            print('Login failed!')
            return (None, None)
        accessToken = GirderAPI.getAccessToken(
            serverAddress, loginState['key'])
...
def _promptLogin():
    ...
    if 'SW_PORTAL_LOGIN' in os.environ:
        ...
        usernamePasswordHash = base64.b64encode(
            combined.encode()).decode("ascii")
        try:
    ...
    try:
        apiKey = GirderAPI.getApiKey(
            serverAddress, basicAuthToken, _API_KEY_NAME)
    except ValueError as e:
        apiKey = GirderAPI.createApiKey(
            serverAddress, basicAuthToken, _API_KEY_NAME)
...
def _downloadFolder(accessToken, path, folder, parsedFileList=None):
    ...
    items = GirderAPI.listItemsInFolder(
        serverAddress, accessToken, folder['_id'])
    for item in items:
        subset = None if not parsedFileList else [
            elem for elem in parsedFileList if len(elem) == 1 and elem[0] == item['name']]
    ...
    for subfolder in GirderAPI.getFolderList(serverAddress, accessToken, parentType='folder', parentId=folder['_id']):
        subset = None if not parsedFileList else [
            elem[1:] for elem in parsedFileList if len(elem) > 1 and elem[0] == subfolder['name']]
        if subset is None or len(subset) > 0:
            _downloadFolder(accessToken, path + '/' +
                            subfolder['name'], subfolder, subset)
...
def downloadDataset(accessToken, datasetName, destinationPath, fileList=None):
    ...
    useCaseCollection = GirderAPI.getCollectionInfo(
        serverAddress, accessToken, _USE_CASE_DATA_COLLECTION)
    # 2 get info of the dataset folder in that collection
    datasetFolder = GirderAPI.getFolderInfo(
        serverAddress, accessToken, parentType='collection', parentId=useCaseCollection['_id'], folderName=datasetName)
...
def downloadDatasetZip(accessToken, datasetName, destinationPath):
    ...
    GirderAPI.downloadFolder(serverAddress, accessToken,
                             path=destinationPath, folderInfo=datasetFolder)


def _uploadFolder(accessToken, folderName, folderPath, parentId, parentType, existingFileList=[]):
    try:
        folderInfo = GirderAPI.getFolderInfo(
            serverAddress, accessToken, parentType=parentType, parentId=parentId, folderName=folderName)
        print('Skip creating folder ' + folderPath)
    except:
        GirderAPI.createFolder(serverAddress, accessToken,
                               parentId, folderName, parentType=parentType)
    ...
    for item in os.listdir(folderPath):
        ...
        if os.path.isfile(itempath):
            if itempath in existingFileList:
                ...
                GirderAPI.uploadFile(
                    serverAddress, accessToken, folderInfo['_id'], item, itempath, parentType='folder')
        else:
            _uploadFolder(accessToken, item, itempath,
                          folderInfo['_id'], 'folder', existingFileList)
...
def uploadDataset(accessToken, datasetName, datasetPath):
    ...
    if datasetName in getDatasetList(accessToken):
        ...
        existingFileList = [os.path.join(datasetPath, f)
...
def getDatasetList(accessToken):
    useCaseCollection = GirderAPI.getCollectionInfo(
        serverAddress, accessToken, _USE_CASE_DATA_COLLECTION)
    jsonList = GirderAPI.getFolderList(
        serverAddress, accessToken, 'collection', useCaseCollection['_id'])
...
def getFileList(accessToken, datasetName):
    ...
    items = GirderAPI.listItemsInFolder(
        serverAddress, accessToken, datasetFolder['_id'])
    subfolders = [(sf, '') for sf in GirderAPI.getFolderList(
        serverAddress, accessToken, parentType='folder', parentId=datasetFolder['_id'])]
    ...
    while len(subfolders) > 0:
        ...
        items = GirderAPI.listItemsInFolder(
            serverAddress, accessToken, subfolder[0]['_id'])
        itemPaths += [path + element['name'] for element in items]
        subfolders = [(sf, path) for sf in GirderAPI.getFolderList(
            serverAddress, accessToken, parentType='folder', parentId=subfolder[0]['_id'])] + subfolders

...
def _downloadFolder(accessToken, path, folder, parsedFileList=None):
    ...
    for item in items:
        subset = (
            None
            if not parsedFileList
            else [
                elem
                for elem in parsedFileList
    ...
    for subfolder in GirderAPI.getFolderList(
        serverAddress, accessToken, parentType="folder", parentId=folder["_id"]
    ):
        subset = (
            None
            if not parsedFileList
        ...
        if subset is None or len(subset) > 0:
            _downloadFolder(
                accessToken, path + "/" + subfolder["name"], subfolder, subset
            )
...
def downloadDatasetZip(accessToken, datasetName, destinationPath):
    ...
    GirderAPI.downloadFolder(
        serverAddress, accessToken, path=destinationPath, folderInfo=datasetFolder
    )


def _uploadFolder(
    accessToken, folderName, folderPath, parentId, parentType, existingFileList=[]
):
    try:
        ...
        GirderAPI.createFolder(
            serverAddress, accessToken, parentId, folderName, parentType=parentType
        )
...
def uploadDataset(accessToken, datasetName, datasetPath):
    ...
    if datasetName in getDatasetList(accessToken):
        ...
        existingFileList = [
            os.path.join(datasetPath, f) for f in getFileList(accessToken, datasetName)
        ]
...
def getFileList(accessToken, datasetName):
    ...
    subfolders = [
        (sf, "")
        for sf in GirderAPI.getFolderList(
            serverAddress,
            accessToken,
            parentType="folder",

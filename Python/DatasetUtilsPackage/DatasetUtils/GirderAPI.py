...
def _makeRequest(
    requestFunction, url, params, headers, actionMessage, data, printError=True
):
    if data:
        response = requestFunction(
            url=url, params=params, headers=headers, data=data, stream=True
        )
    else:
    ...
    if printError:
        ...
        _writeToErrorLog(
            {
                "function": "POST"
                if requestFunction is requests.post
                else "GET"
                if requestFunction is requests.get
                else "OTHER",
                "url": url,
                "params": params,
                "headers": headers,
    ...
    raise ValueError(
        "Response code %s while %s", str(response.status_code), actionMessage
    )
...
def createApiKey(serverAddress, basicAuthToken, apiKeyName):
    response = _makePostRequest(
        ...
        params={
            "name": apiKeyName,
            "scope": '["core.data.read", "core.data.write"]',
            "tokenDuration": "1",
        },
...
def getFolderList(serverAddress, accessToken, parentType, parentId, folderName=None):
    ...
    response = _makeGetRequest(
        ...
        params=params,
        headers={"Girder-Token": accessToken},
        actionMessage="getting folder list for: %s, %s, %s"
        % (parentType, parentId, folderName),
    )
...
def getFolderInfo(serverAddress, accessToken, parentType, parentId, folderName):
    folderList = getFolderList(
        serverAddress, accessToken, parentType, parentId, folderName
    )
...
def _printProgress(fileName, progressBytes, totalBytes=None):
    ...
    if totalBytes is None:
        ...
        stdout.write(
            "\r%s [%d/%d %s]"
            % (fileName, progressBytes / divider, totalBytes / divider, unit)
        )
...
def uploadFile(serverAddress, accessToken, parentId, name, path, parentType="folder"):
    ...
    with open(path, "rb") as filehandle:
        stdout.write(
            ...
            % (
                path,
                chunkIndex * _MB_PER_CHUNK,
                ceil(filesize / _CHUNK_SIZE * _MB_PER_CHUNK),
            )
        )

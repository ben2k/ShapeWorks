...
def _makeRequest(requestFunction, url, params, headers, actionMessage, data, printError=True):
    ...
    raise ValueError('Response code %s while %s', str(
        response.status_code), actionMessage)
...
def createApiKey(serverAddress, basicAuthToken, apiKeyName):
    response = _makePostRequest(
        ...
        params={'name': apiKeyName,
                'scope': '[\"core.data.read\", \"core.data.write\"]', 'tokenDuration': '1'},
...
def _printProgress(fileName, progressBytes, totalBytes=None):
    ...
    if totalBytes is None:
        stdout.write('\r%s [%d %s]  ' %
                     (fileName, progressBytes/divider, unit))
    else:
        stdout.write('\r%s [%d/%d %s]' % (fileName,
                     progressBytes/divider, totalBytes/divider, unit))
...
def uploadFile(serverAddress, accessToken, parentId, name, path, parentType='folder'):
    ...
    response = _makePostRequest(
        ...
        params={'parentId': parentId, 'name': name, 'parentType': parentType,
                'size': filesize, 'mimeType': 'application/octet-stream'},
        headers={'Girder-Token': accessToken},
    ...
    with open(path, 'rb') as filehandle:
        stdout.write('\r%s [%d/%d MB]' % (path, chunkIndex *
                     _MB_PER_CHUNK, ceil(filesize / _CHUNK_SIZE * _MB_PER_CHUNK)))
        ...
        while(True):
            ...
            if chunk:
                response = _makePostRequest(
                    ...
                    headers={'Girder-Token': accessToken},
                    actionMessage='Uploading chunk %d for %s' % (
                        chunkIndex, name),
                    data=chunk
                )
                offset += len(chunk)

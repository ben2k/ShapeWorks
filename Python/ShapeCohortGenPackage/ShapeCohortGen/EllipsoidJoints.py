...
def generate_ellipsoid_joint(num_samples, meshDir, randomize_center, randomize_x_radius, randomize_y_radius, randomize_z_radius, mode_size, mode_rotation, separation=0):
    ...
    for i in range(num_samples):
        ...
        # ellipsoid 1
        ellipsoid_joint1 = addEllipsoid(
            center_loc_1, radii_1, 0, rotation_axis='Y')
        # Write to .ply file
        ...
        # ellipsoid 2
        ellipsoid_joint2 = addEllipsoid(
            center_loc_2, radii_2, rotation, rotation_axis='Y')
        # write to ply
        ply_writer = vtk.vtkPLYWriter()
...
def generate(num_samples, out_dir, randomize_center, randomize_x_radius, randomize_y_radius, randomize_z_radius, mode_size, mode_rotation, separation):
    if(mode_size == False and mode_rotation == False):
        raise Exception(
            "The joint cohort generation should have atleast one mode of variation. Please turn on either mode_size or mode_rotation or both")

    meshDir = out_dir + "meshes/"
    make_dir(meshDir)

    generate_ellipsoid_joint(num_samples, meshDir, randomize_center, randomize_x_radius,

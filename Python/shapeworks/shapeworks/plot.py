...
def add_volume_to_plotter(
    pvPlotter,  # pyvista plotter
    vtkImg,  # vtk image to be added
    rowIdx,
    colIdx,  # subplot row and column index
...
def plot_volumes(
    volumeList,  # list of shapeworks images to be visualized
    ...
    if volumeNames is not None:
        if use_same_window and (len(volumeNames) > 1):
            print(
                "A single title needed when all volumes are to be displayed on the same window"
            )
            return
    ...
    # define plotter
    plotter = pv.Plotter(
        shape=(grid_rows, grid_cols), notebook=is_interactive, border=show_borders
    )
...
def plot_meshes(
    meshList,  # list of shapeworks meshes to be visualized
    ...
    if meshNames is not None:
        if use_same_window and (len(meshNames) > 1):
            print(
                "A single title needed when all meshes are to be displayed on the same window"
            )
            return
...
def plot_meshes_volumes_mix(
    objectList,  # list of shapeworks meshes to be visualized
    ...
    if objectNames is not None:
        if use_same_window and (len(objectNames) > 1):
            print(
                "A single title needed when all objects are to be displayed on the same window"
            )
            return
...
def pca_loadings_violinplot(loadings, cumulative_variance, shape_models_dir):
    ...
    ax = sns.violinplot(
        x="PCA Mode", y="PCA Score", data=df, palette="cool_r", split=True, scale="area"
    )
    fig = plt.gcf()

...
def generate_download_flag(outputDirectory, folder):
    ...
    if os.path.exists(outputDirectory + folder):
        ...
        if (
            len(os.listdir(outputDirectory + folder)) == 0
            or len(os.listdir(outputDirectory + folder)) < 3
        ):
            download_flag = True
        else:
            print(
...
def download_subset(use_case, datasetName, outputDirectory):
    ...
    if use_case in ["ellipsoid", "ellipsoid_cut", "left_atrium"]:
        if generate_download_flag(outputDirectory, "segmentations"):
            segFilesList = sorted(
                [
                    ...
                    if re.search("^segmentations(?:/|\\\).*nrrd$", files)
                ]
            )[:3]
        ...
        if generate_download_flag(outputDirectory, "meshes"):
            meshFilesList = sorted(
                [
                    files
                    for files in fileList
                    if re.search("^meshes(?:/|\\\).*ply$", files)
                ]
            )[:3]
            DatasetUtils.downloadDataset(
    ...
    if use_case in ["femur", "femur_cut", "left_atrium"]:
        if generate_download_flag(outputDirectory, "images"):
            imageFilelist = sorted(
                [
                    files
                    for files in fileList
                    if re.search("^images(?:/|\\\).*nrrd$", files)
                ]
            )[:3]
            DatasetUtils.downloadDataset(
        ...
        if generate_download_flag(outputDirectory, "groomed/meshes/"):
            meshFilesList = sorted(
                [
                    files
                    for files in fileList
                    if re.search("^groomed(?:/|\\\)meshes(?:/|\\\).*ply$", files)
                ]
            )[:3]
            DatasetUtils.downloadDataset(
                datasetName, destinationPath=outputDirectory, fileList=meshFilesList
        ...
        if generate_download_flag(outputDirectory, "groomed/images/"):
            imageFilesList = sorted(
                [
                    files
                    for files in fileList
                    if re.search("^groomed(?:/|\\\)images(?:/|\\\).*nrrd$", files)
                ]
            )[:7]
            DatasetUtils.downloadDataset(
        ...
        if generate_download_flag(outputDirectory, "groomed/distance_transforms/"):
            dtFilesList = sorted(
                [
                    ...
                    for files in fileList
                    if re.search(
                        "^groomed(?:/|\\\)distance_transforms(?:/|\\\).*nrrd$", files
                    )
                ]
            )[:7]
            DatasetUtils.downloadDataset(
        ...
        if generate_download_flag(outputDirectory, "shape_models/femur/1024/"):
            wolrdFilesList = sorted(
                [
                    files
                    for files in fileList
                    if re.search(
                        "^shape_models(?:/|\\\)femur(?:/|\\\)1024(?:/|\\\).*world.particles$",
                        files,
                    )
                ]
            )[:7]
            ...
            localFilesList = sorted(
                [
                    files
                    for files in fileList
                    if re.search(
                        "^shape_models(?:/|\\\)femur(?:/|\\\)1024(?:/|\\\).*local.particles$",
                        files,
                    )
                ]
            )[:7]
        ...
        if generate_download_flag(outputDirectory, "contours"):
            contourFilesList = sorted(
                [
                    files
                    for files in fileList
                    if re.search("^contours(?:/|\\\).*vtp$", files)
                ]
            )[:3]
            DatasetUtils.downloadDataset(
    ...
    if domains_per_shape == 1:
        ...
        for i in range(len(inDataList)):
            ...
            for j in range(i, len(inDataList)):
                ...
                x, y, z = (
                    max(x, image_2.shape[0]),
                    max(y, image_2.shape[1]),
                    max(z, image_2.shape[2]),
                )
                image_1 = np.pad(
                    image_1,
                    (
                        (
                            (x - image_1.shape[0]) // 2,
                            (x - image_1.shape[0]) - (x - image_1.shape[0]) // 2,
                        ),
                        (
                            (y - image_1.shape[1]) // 2,
                            (y - image_1.shape[1]) - (y - image_1.shape[1]) // 2,
                        ),
                        (
                            (z - image_1.shape[2]) // 2,
                            (z - image_1.shape[2]) - (z - image_1.shape[2]) // 2,
                        ),
                    ),
                )
                image_2 = np.pad(
                    ...
                    (
                        (
                            (x - image_2.shape[0]) // 2,
                            (x - image_2.shape[0]) - (x - image_2.shape[0]) // 2,
                        ),
                        (
                            (y - image_2.shape[1]) // 2,
                            (y - image_2.shape[1]) - (y - image_2.shape[1]) // 2,
                        ),
                        (
                            (z - image_2.shape[2]) // 2,
                            (z - image_2.shape[2]) - (z - image_2.shape[2]) // 2,
                        ),
                    ),
                )

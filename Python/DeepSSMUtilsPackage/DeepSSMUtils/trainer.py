...
def supervised_train(config_file):
    ...
    loss_func = method_to_call = getattr(
        losses, parameters["loss"]["function"])
    # load the loaders
    ...
    for i in range(num_pca):
        temp = np.loadtxt(
            aug_dir + '/PCA_Particle_Info/pcamode' + str(i) + '.particles')
        orig_pc[i, :] = temp.flatten()

    bias = torch.from_numpy(orig_mean.flatten()).to(
        device)  # load the mean here
    weight = torch.from_numpy(orig_pc.T).to(
        device)  # load the PCA vectors here
    ...
    log_print(logger, ["Epoch", "LR", "Train_Err",
              "Train_Rel_Err", "Val_Err", "Val_Rel_Err", "Sec"])
    t0 = time.time()
    ...
    for e in range(1, num_epochs + 1):
        ...
        for img, pca, mdl in train_loader:
            ...
            train_rel_loss = loss_func(
                pred_pca, pca) / loss_func(pred_pca*0, pca)
        ...
        if ((e % eval_freq) == 0 or e == 1):
            ...
            log_print(logger, [e, scheduler.get_lr()[
                      0], train_mr_MSE, train_rel_err, val_mr_MSE, val_rel_err, time.time()-t0])
            # save
            if val_rel_err < best_val_rel_error:
                best_val_rel_error = val_rel_err
                best_epoch = e
                torch.save(net.state_dict(), os.path.join(
                    model_dir, 'best_model.torch'))
            t0 = time.time()
        if decay_lr:
            scheduler.step()
    logger.close()
    ...
    if fine_tune:
        ...
        log_print(logger, ["Epoch", "Train_Err_mdl", "Train_Rel_Err_mdl",
                  "Val_Err_mdl", "Val_Rel_Err_mdl", "Sec"])
        ...
        loss_func = method_to_call = getattr(
            losses, parameters['fine_tune']["loss"])
        ...
        for e in range(1, ft_epochs + 1):
            ...
            for img, pca, mdl in train_loader:
                ...
                opt.step()
                train_losses.append(loss.item())
                train_rel_loss = F.mse_loss(
                    pred_mdl, mdl) / F.mse_loss(pred_mdl*0, mdl)
                train_rel_losses.append(train_rel_loss.item())
            # test validation
            if ((e % eval_freq) == 0 or e == 1):
                net.eval()
                ...
                for img, pca, mdl in val_loader:
                    ...
                    val_losses.append(v_loss.item())
                    val_rel_loss = (F.mse_loss(pred_mdl, mdl) /
                                    F.mse_loss(pred_mdl*0, mdl)).item()
                    val_rel_losses.append(val_rel_loss)
                    if val_rel_loss < best_ft_val_rel_error:
                        best_ft_val_rel_error = val_rel_loss
                        best_ft_epoch = e
                        torch.save(net.state_dict(), os.path.join(
                            model_dir, 'best_model_ft.torch'))
                # log
                train_mr_MSE = np.mean(np.sqrt(train_losses))
                val_mr_MSE = np.mean(np.sqrt(val_losses))
                train_rel_err = np.mean(train_rel_losses)
                val_rel_err = np.mean(val_rel_losses)
                log_print(logger, [e, train_mr_MSE, train_rel_err,
                          val_mr_MSE, val_rel_err, time.time()-t0])
                t0 = time.time()

        logger.close()
        torch.save(net.state_dict(), os.path.join(
            model_dir, 'final_model_ft.torch'))
        parameters['best_ft_model_epochs'] = best_ft_epoch
        with open(config_file, "w") as json_file:
            json.dump(parameters, json_file, indent=2)
        print("Fine tuning complete, model saved. Best model after epoch " +
              str(best_ft_epoch))

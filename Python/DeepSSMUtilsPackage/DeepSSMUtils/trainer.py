    ...
    for e in range(1, num_epochs + 1):
        ...
        if (e % eval_freq) == 0 or e == 1:
            ...
            if val_rel_err < best_val_rel_error:
                ...
                best_epoch = e
                torch.save(
                    net.state_dict(), os.path.join(model_dir, "best_model.torch")
                )
            t0 = time.time()
        if decay_lr:
            scheduler.step()
    ...
    if fine_tune:
        ...
        for e in range(1, ft_epochs + 1):
            ...
            for img, pca, mdl in train_loader:
                ...
                opt.step()
                train_losses.append(loss.item())
                train_rel_loss = F.mse_loss(pred_mdl, mdl) / F.mse_loss(
                    pred_mdl * 0, mdl
                )
                train_rel_losses.append(train_rel_loss.item())
            # test validation
            if (e % eval_freq) == 0 or e == 1:
                ...
                for img, pca, mdl in val_loader:
                    ...
                    val_losses.append(v_loss.item())
                    val_rel_loss = (
                        F.mse_loss(pred_mdl, mdl) / F.mse_loss(pred_mdl * 0, mdl)
                    ).item()
                    ...
                    if val_rel_loss < best_ft_val_rel_error:
                        ...
                        torch.save(
                            net.state_dict(),
                            os.path.join(model_dir, "best_model_ft.torch"),
                        )
                # log
                train_mr_MSE = np.mean(np.sqrt(train_losses))
                val_mr_MSE = np.mean(np.sqrt(val_losses))
        ...
        with open(config_file, "w") as json_file:
            json.dump(parameters, json_file, indent=2)
        print(
            "Fine tuning complete, model saved. Best model after epoch "
            + str(best_ft_epoch)
        )

...
class ConvolutionalBackbone(nn.Module):
    def __init__(self, img_dims):
        ...
        padvals = [4, 8, 8]
        for i in range(3):
            self.out_fc_dim[0] = net_utils.poolOutDim(
                self.out_fc_dim[0] - padvals[i], 2
            )
            self.out_fc_dim[1] = net_utils.poolOutDim(
                self.out_fc_dim[1] - padvals[i], 2
            )
            self.out_fc_dim[2] = net_utils.poolOutDim(
                self.out_fc_dim[2] - padvals[i], 2
            )
...
class DeterministicEncoder(nn.Module):
    def __init__(self, num_latent, img_dims, loader_dir):
        ...
        self.pca_pred = nn.Sequential(
            OrderedDict([("linear", nn.Linear(96, self.num_latent))])
        )

    def forward(self, x):
        x = self.ConvolutionalBackbone(x)
        pca_load = self.pca_pred(x)
        pca_load_unwhiten = net_utils.unwhiten_PCA_scores(
            pca_load, self.loader_dir, self.device
        )
    ...
    def __init__(self, config_file):
        ...
        if parameters["encoder"]["deterministic"]:
            self.encoder = DeterministicEncoder(
                self.num_latent, self.img_dims, self.loader_dir
            )
        if not self.encoder:
        ...
        if parameters["decoder"]["deterministic"]:
            if parameters["decoder"]["linear"]:
                self.decoder = DeterministicLinearDecoder(
                    self.num_latent, self.num_corr
                )
        if not self.decoder:

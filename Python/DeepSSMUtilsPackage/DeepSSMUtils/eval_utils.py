...
def get_distance_meshes(out_dir, DT_dir, prediction_dir, mean_prefix):
    ...
    orig_mesh_list = sorted(get_mesh_from_DT(
        DT_files, out_dir + "OriginalMeshes/"))
    ...
    pred_mesh_list = sorted(get_mesh_from_particles(
        particle_files, out_dir + "PredcitedMeshes/", mean_prefix))
    # Step 4: Get distance between original and predicted mesh
    print("\n\nGetting distance between original and predicted meshes...")
    orig_dist_mesh_list, pred_dist_mesh_list, avg_distance = surface_to_surface_distance(
        orig_mesh_list, pred_mesh_list, out_dir)
    print("Done.\n")
    return avg_distance
...
def surface_to_surface_distance(orig_list, pred_list, out_dir):
    ...
    for index in range(len(orig_list)):
        ...
        dist1 = orig_mesh.distance(pred_mesh).write(
            orig_out).getFieldMean("distance")
        dist2 = pred_mesh.distance(orig_mesh).write(
            pred_out).getFieldMean("distance")

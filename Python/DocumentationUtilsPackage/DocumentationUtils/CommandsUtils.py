...
def getCommandHelp(cmd, verbose=False):
    out = subprocess.Popen(
        cmd + " --help", stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True)
...
def addOptions(mdFile, stdout, lid, spacedelim=' '):
    ...
    while True:
        ...
        optname, optdesc, lid = getNameAndDescription(
            stdout, lid, spacedelim=spacedelim)
...
def getSubCommands(stdout, lid, spacedelim=' '):
    ...
    while True:
        ...
        if line[0] != ' ':
            ...
            cmdname, desc, lid = getNameAndDescription(
                stdout, lid, spacedelim=spacedelim)
    ...
    while lid < nlines:
        ...
        if 'Usage' in line:
            ...
            if len(cmdsplit) == 0:
                ...
                # remove the last command as it is included in the usage
                cmd_prefix = ' '.join(cmdsplit)
        ...
        if 'Available commands' in line:
            subcommands, lid = getSubCommands(
                stdout, lid, spacedelim=spacedelim)
            for cmdtype in subcommands.keys():
                ...
                for cmdname in subcommands[cmdtype].keys():
                    addCommand(mdFile, cmd + " " + cmdname, level=level +
                               1, spacedelim=spacedelim, verbose=verbose)
                    mdFile.new_line(
                        '[Back to ' + cmdtype + ' Commands' + '](#' + cmdtype.lower() + '-commands)')
        lid += 1
# %%

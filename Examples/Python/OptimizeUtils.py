...
def get_parameter_text(parameterKey, parameterValue, domains_per_shape):
    ...
    assert(len(parameterValue) == domains_per_shape), str(parameterKey) + \
        " parameter should be an array of length = domains_per_shape= " + \
...
def create_SWRun_xml(xmlfilename, inDataFiles, parameterDictionary, outDir):
    ...
    number_of_particles.text = get_parameter_text(
        'number_of_particles', parameterDictionary['number_of_particles'], parameterDictionary['domains_per_shape'])
    use_normals = ET.SubElement(root, 'use_normals')
    use_normals.text = get_parameter_text(
        'use_normals', parameterDictionary['use_normals'], parameterDictionary['domains_per_shape'])
    normal_weight = parameterDictionary['normal_weight']
    attribute_scales = ET.SubElement(root, 'attribute_scales')
    attribute_scales.text = get_attribute_scales_text(
        parameterDictionary['use_normals'], parameterDictionary['normal_weight'], parameterDictionary['domains_per_shape'])
    ...
    recompute_regularization_interval = ET.SubElement(
        root, 'recompute_regularization_interval')
    ...
    initial_relative_weighting = ET.SubElement(
        root, 'initial_relative_weighting')
    ...
    if 'use_shape_statistics_after' in parameterDictionary:
        use_shape_statistics_after = ET.SubElement(
            root, 'use_shape_statistics_after')
    ...
    parameterValue = [1 for _ in range(
        parameterDictionary['domains_per_shape'])]
    use_xyz.text = get_parameter_text(
        'use_xyz', parameterValue, parameterDictionary['domains_per_shape'])
    ...
    if 'visualizer_screenshot_directory' in parameterDictionary:
        ...
        visualizer_screenshot_directory = ET.SubElement(
            root, 'visualizer_screenshot_directory')
    ...
    if 'cutting_plane_counts' in parameterDictionary and 'cutting_planes' in parameterDictionary:
        ...
        for i in range(len(cutting_planes_var)):
            ...
            cutting_planes_text += "  " + str(cur_plane[0][0]) + " " + str(cur_plane[0][1]) + " " + str(cur_plane[0][2]) + " " + str(cur_plane[1][0]) + " " + str(
                cur_plane[1][1]) + " " + str(cur_plane[1][2]) + " " + str(cur_plane[2][0]) + " " + str(cur_plane[2][1]) + " " + str(cur_plane[2][2]) + "\n"
        for i in range(len(cutting_plane_counts_var)):
            cutting_plane_counts_text += "  " + \
                str(cutting_plane_counts_var[i]) + "\n"
    ...
    if 'geodesics_cache_size_multiplier' in parameterDictionary:
        geodesics_cache_size_multiplier = ET.SubElement(
            root, 'geodesics_cache_size_multiplier')
...
def runShapeWorksOptimize(parentDir, inDataFiles, parameterDictionary):
    ...
    numP = str(numP)
    numP = numP.replace('[', "").replace(']', "").replace(
        ",", "").replace(" ", "_")
    ...
    outPointsLocal, outPointsWorld = _convertFilenamesToPointFilenames(
        inDataFiles, outDir)
...
def runShapeWorksOptimize_FixedDomains(parentDir, inDataFiles, parameterDictionary):
    ...
    parameterFile = parentDir + "correspondence_" + str(numP) + ".xml"
    create_SWRun_fixed_domains(
        parameterFile, inDataFiles, parameterDictionary, outDir, numFD, inparts)
    create_cpp_xml(parameterFile, parameterFile)
    print(parameterFile)
    execCommand = ["shapeworks", "optimize", "--name=" + parameterFile]
    subprocess.check_call(execCommand)

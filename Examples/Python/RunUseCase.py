...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "\tSee http://sciinstitute.github.io/ShapeWorks/users/install.html for more information"
        )
        print(
    ...
    parser.add_argument(
        "use_case",
        help="Must specify which of these use cases to run.",
        choices=[
            "ellipsoid",
            "ellipsoid_evaluate",
            "ellipsoid_mesh",
            ...
            "femur_cut",
            "deep_ssm",
            "supershapes_1mode_contour",
            "thin_cavity_bean",
        ],
    )
    parser.add_argument(
        "--use_subsample",
        help="Run the pipeline for a subset of data",
        action="store_true",
    )
    parser.add_argument(
        "--num_subsample",
        help="Size of subset to run on (default: %(default)s)",
        type=int,
        default=3,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--skip_grooming",
        help="Skip the grooming steps and start with already prepped (i.e., groomed) data",
        action="store_true",
    )
    parser.add_argument(
        "--groom_images",
        help="Apply grooming steps to both the shapes (segmentations or surface meshes) and raw images",
        action="store_true",
    )
    parser.add_argument(
        "--use_single_scale",
        help="Use single scale optimization (default: multi scale)",
        action="store_true",
    )

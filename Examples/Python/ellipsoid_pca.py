...
def Run_Pipeline(args):
    ...
    for subdir in (['pca']):
        Path(ellipsoids_dir).joinpath(Path(subdir)).mkdir(
    ...
    print("\nThe sample size of the dataset is : ",
          shape_statistics.sampleSize())
    ...
    if len(local_point_files) > 3 and not args.tiny_test:
        sw.plot.pca_loadings_violinplot(
            pca_loadings, cumulative_variance, pca_dir)
        sw.plot.plot_pca_metrics(
            cumulative_variance, explained_variance, pca_dir)
